define(['core/ajax', 'core/str'], function(Ajax, Str) {
    ...
    function submitNLQ() {
        ...
        if (!input || !response || !spinner) return;
        ...
        Ajax.call([{
            ...
            args: {courseid: cid, query: q, risklevel: filterMode, limit: 50}
        }])[0].done(function(data) {
            spinner.style.display = 'none';
            if (data.ai_insight) {
                response.innerHTML = '<strong>AI Insight:</strong> ' + escapeHtml(data.ai_insight);
            ...
            spinner.style.display = 'none';
            response.innerHTML = '<span style="color:var(--u-ter);">Failed to query AI service. Check that the service is running.</span>';
            response.style.display = 'block';
        });
    }
