...
function _umatStripQuizFromText(txt){var m=txt.match(/\`\`\`(?:json)?\s*(\{[^`]*?"quiz"\s*:[^`]*?\})\s*\`\`\`/s);if(!m)return{text:txt,quiz:null};try{var data=JSON.parse(m[1]);if(data&&data.quiz&&data.quiz.questions&&data.quiz.questions.length){return{text:txt.replace(m[0],'').trim(),quiz:{quiz:data.quiz}};}}catch(e){}return{text:txt,quiz:null};}
function _umatCleanPreview(txt,fallback){if(!txt)return fallback||'';var r=_umatStripQuizFromText(txt);return r.text||fallback||'';}
