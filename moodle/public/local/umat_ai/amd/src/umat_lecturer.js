...
function renderLecCourses(courses,g){
  ...
  /* Tile body click -> Insights dashboard (instant, cache-backed) */
  g.querySelectorAll('.yt-tile').forEach(function(tile){
    tile.addEventListener('click',function(e){
      ...
      /* Route straight to the redesigned Insights dashboard with the course
         selected — the AMD module paints instantly from its per-course cache. */
      if(window.selectInsightsCourse)window.selectInsightsCourse(CID,CN);
      closePanel();lecOv.classList.add('open');switchPane('lec-insights');updateBodyLock();
    });
