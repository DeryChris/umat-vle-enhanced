...
/* ï¿½"?ï¿½"?ï¿½"? LECTURER COURSE TILES ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"? */
    ...
    return'<div class="yt-tile" data-cid="'+c.id+'" data-cname="'+esc(c.fullname||'')+'">'+
      ...
      '<div class="yt-meta">'+
        ...
        '<div class="yt-text">'+
          ...
          '<p class="yt-channel">'+esc(c.shortname||'')+(enrolled?' Aï¿½ '+enrolled+' students':'')+'</p>'+
          '<p class="yt-stats">'+sessions+' sessions'+(pending>0?' Aï¿½ '+pending+' outputs pending':'')+'</p>'+
        '</div>'+
      '</div>'+
...
function renderLcpFeature(name){
  ...
  var body=document.getElementById('lcp-feature-body');body.innerHTML='<div class="umat-empty"><span class="material-symbols-outlined">hourglass_empty</span><p>Loading '+meta[1].toLowerCase()+'ï¿½?ï¿½</p></div>';
...
function loadAnalytics(cid){
  ...
  if(cid){
    ...
    document.getElementById('lec-an-course-label').textContent=cid===CID?CN:'Loadingï¿½?ï¿½';
    ajax('local_umat_ai_get_analytics',{courseid:cid,days:30},function(d){
      var s=function(id,v){var e=document.getElementById(id);if(e)e.textContent=v;};
...
/* ï¿½"?ï¿½"? Render Analytics Overview (all courses) ï¿½"?ï¿½"? */
...
function buildHeatmap(daily,maxV,struggleIdx){
  ...
  days.forEach(function(day,row){
    ...
    for(var col=0;col<n;col++){
      ...
      html+='<div class="umat-hm-cell" style="background:'+bg+';color:'+color+';" title="'+day+' Aï¿½ L'+(col+1)+': '+val+'">'+(val>0?val:'')+'</div>';
    }
  });
  grid.innerHTML=html;
...
/* Library ï¿½?" with course overlay selector */
...
function loadLibrary(cid){
  ...
  if(hdr){
    ...
    hdr.innerHTML=(course?'<button class="umat-lib-sel-label" id="lec-lib-sel-label" type="button"><span class="material-symbols-outlined">menu_book</span>'+esc(course.shortname)+'</button>':'')+
      '<input type="text" id="lec-lib-search" placeholder="Search materialsï¿½?ï¿½" style="padding:6px 12px;border:1px solid var(--u-olv);border-radius:var(--u-rp);font-size:12px;outline:none;font-family:inherit;color:var(--u-ons);background:var(--u-sfl);width:min(140px,35vw);">';
    var lbl=document.getElementById('lec-lib-sel-label');
    if(lbl)lbl.addEventListener('click',openLecLibPicker);
  }
  g.innerHTML='<div class="umat-empty" style="grid-column:1/-1;"><span class="material-symbols-outlined">hourglass_empty</span><p>Loading materialsï¿½?ï¿½</p></div>';
  ajax('local_umat_ai_get_course_materials',{courseid:courseId},function(r){renderLibTiles(r.materials||[],g);if(typeof updateMaterialAnalysis==='function')updateMaterialAnalysis(courseId);if(typeof updateVideoGenerationStatus==='function')updateVideoGenerationStatus(courseId);},function(){g.innerHTML='<div class="umat-empty" style="grid-column:1/-1;"><span class="material-symbols-outlined">error_outline</span><p>Could not load materials.</p></div>';});
}
...
/* Sessions ï¿½?" with course overlay selector */
...
/* Analytics ï¿½?" course overlay selector */
...
function loadSessions(cid){
  ...
  list.innerHTML='<div class="umat-empty"><span class="material-symbols-outlined">hourglass_empty</span><p>Loading sessionsï¿½?ï¿½</p></div>';
...
function loadReviewPane(){
  ...
  body.innerHTML='<div class="umat-empty"><span class="material-symbols-outlined">hourglass_empty</span><p>Loading pending outputsï¿½?ï¿½</p></div>';
...
/* ï¿½"?ï¿½"?ï¿½"? Student Issues (Lecturer) ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"?ï¿½"? */
function loadLecturerIssues(){
  ...
  body.innerHTML='<div class="umat-empty"><span class="material-symbols-outlined">hourglass_empty</span><p>Loading issuesï¿½?ï¿½</p></div>';
  ...
  ajax('local_umat_ai_get_course_issues',args,function(r){
    ...
    body.innerHTML=issues.map(function(iss){
      ...
      return '<div class="umat-issue-card" data-id="'+iss.id+'" style="background:var(--u-sflo);border:1px solid var(--u-olv);border-radius:var(--u-r12);padding:14px;margin-bottom:10px;">'
        ...
        +'<div><strong style="font-size:13px;">'+esc(iss.fullname||'Student')+'</strong><span style="font-size:10px;color:var(--u-ol);display:block;">'+catLabel+(iss.topic?' Aï¿½ '+esc(iss.topic):'')+' Aï¿½ '+ago+'</span></div></div>'
        ...
        +'<textarea class="umat-issue-resp-ta" data-id="'+iss.id+'" placeholder="Write a replyï¿½?ï¿½" rows="2" style="width:100%;padding:8px;font-size:12px;border:1px solid var(--u-olv);border-radius:var(--u-r6);resize:vertical;">'+(iss.lecturer_response?'':'')+'</textarea>'
    ...
    /* Wire response toggle ï¿½?" show reply box */
    ...
    /* Wire response save ï¿½?" inline update, no full reload */
    body.querySelectorAll('.umat-issue-save-resp').forEach(function(btn){
      btn.addEventListener('click',function(){
        ...
        btn.disabled=true;btn.textContent='Sendingï¿½?ï¿½';
        ajax('local_umat_ai_update_issue_response',{issue_id:parseInt(id),response:txt},function(r){
          ...
          /* Inline update ï¿½?" keep card, just refresh the display */
...
/* ï¿½"?ï¿½"? Render Struggle Overview (all courses) ï¿½"?ï¿½"? */
function renderStruggleOverview(agg){
  ...
  document.getElementById('ov-stru-bars').innerHTML=agg.per_course.sort(function(a,b){return b.questions-a.questions;}).map(function(c){
    ...
    var struggleLabel=typeof c.struggle==='number'?c.struggle+'/100':'ï¿½?"';
...
/* ï¿½"?ï¿½"? SVG Score Ring (reusable) ï¿½"?ï¿½"? */
...
/* ï¿½"?ï¿½"? Event Source Bar (visual breakdown for a topic) ï¿½"?ï¿½"? */
...
/* ï¿½"?ï¿½"? Severity Donut Chart (SVG) ï¿½"?ï¿½"? */
...
/* Struggle ï¿½?" course overlay selector */
