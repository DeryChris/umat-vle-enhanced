define([], function () {
    ...
    function viewPdf(container, opts) {
        ...
        function render() {
            ...
            var html =
                '<div class="umat-vw-pdf-toolbar">' +
                    '<div class="umat-vw-pdf-tb-left">' +
                        '<span class="umat-vw-pdf-name">' + esc(name) + '</span>' +
                        (size ? '<span class="umat-vw-pdf-size">' + formatBytes(size) + '</span>' : '') +
                    '</div>' +
                    '<div class="umat-vw-pdf-tb-right">' +
                        '<button class="umat-vw-pdf-btn" id="umat-vw-pdf-dl" title="Download"><span class="material-symbols-outlined">download</span></button>' +
                        '<button class="umat-vw-pdf-btn" id="umat-vw-pdf-tab" title="Open in new tab"><span class="material-symbols-outlined">open_in_new</span></button>' +
                    '</div>' +
                '</div>' +
                '<object data="' + escapedUrl + '" type="application/pdf" class="umat-vw-pdf-object">' +
                    '<div class="umat-vw-empty">' +
                        '<span class="material-symbols-outlined">picture_as_pdf</span>' +
                        '<p>This PDF cannot be displayed in your browser.</p>' +
                        '<a class="umat-vw-btn-link" href="' + escapedUrl + '" download="' + esc(name) + '">Download PDF</a>' +
                    '</div>' +
                '</object>';

            container.innerHTML = html;

            var dlEl = document.getElementById('umat-vw-pdf-dl');
            if (dlEl) dlEl.addEventListener('click', function () {
            ...
            var tabEl = document.getElementById('umat-vw-pdf-tab');
            if (tabEl) tabEl.addEventListener('click', function () {
    ...
    function viewPptx(container, opts) {
        ...
        function buildApiUrl() {
            // Derive Moodle root from the file URL itself (e.g. pluginfile.php URL)
            var a = document.createElement('a');
            a.href = url;
        ...
        function renderLayout() {
            ...
            // Override main Escape handler for fullscreen
        ...
        function renderThumbnails() {
            ...
            sb.innerHTML = slides.map(function (s, i) {
                var idx = i + 1;
                var src = s.src ? s.src.replace('action=slides', 'action=slide') : '';
                return '<div class="umat-vw-pptx-thumb" data-slide="' + idx + '">' +
                    '<img src="' + esc(src) + '" alt="Slide ' + idx + '" loading="lazy">' +
